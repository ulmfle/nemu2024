...
void concat(write_operand_, SUFFIX) (Operand *op, DATA_TYPE src) {
	...
	else if(op->type == OP_TYPE_MEM) { swaddr_write(op->addr, op->size, is_base_sp_bp ? SR_SS : SR_DS, src); if (page_translate(op->addr) == 0x14fffe) assert(hwaddr_read(0x14fffe, 4) == 0x752d3f80);}
	else { assert(0); }
}
